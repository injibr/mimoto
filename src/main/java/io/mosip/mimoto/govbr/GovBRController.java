...
public class GovBRController {
    ...
    public ResponseEntity<ApiResponse<GovBRUserProfileResponse>> getUserProfile(@RequestParam("code") String code, @RequestParam("codeVerifier") String codeVerifier, @RequestParam(value = "redirectUri", required = false) String redirectUri) {
        logger.info("Received request for user profile");
        GovBRUserProfileResponse response = govBRService.getUserProfile(code, codeVerifier, redirectUri);
